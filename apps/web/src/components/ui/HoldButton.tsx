...
const HoldButton: React.FC<HoldButtonProps> = ({
  ...
  return (
    <button
      ...
      type="button"
      disabled={disabled}
      className={`hb-root group relative isolate m-0 inline-grid cursor-none touch-manipulation select-none place-items-center border-0 font-medium leading-none tracking-[0.01em] outline-none [-webkit-tap-highlight-color:transparent] [-webkit-touch-callout:none] [background:var(--hb-bg)] rounded-(--hb-radius) text-(--hb-text) shadow-[inset_0_1px_0_rgba(255,255,255,0.06)] [transition:transform_160ms_var(--hb-ease-out),background-color_160ms_ease,box-shadow_var(--hb-release)_var(--hb-ease-out)] [@media(hover:hover)_and_(pointer:fine)]:enabled:hover:[background:color-mix(in_srgb,var(--hb-bg)_92%,#fff)] data-[data-[phase=holding]:data-[input=pointer]:transform-[scale(var(--hb-press))][glow=true]:data-[phase=holding]:shadow-[${GLOW}] data-[glow=true]:data-[phase=done]:shadow-[${GLOW}] data-[glow=true]:data-[phase=holding]:[transition:transform_160ms_var(--hb-ease-out),background-color_160ms_ease,box-shadow_var(--hb-hold)_linear] focus-visible:[outline:2px_solid_var(--hb-fill)] focus-visible:outline-offset-[3px] disabled:pointer-events-none disabled:cursor-default disabled:opacity-50 contrast-more:[outline:1px_solid_var(--hb-text)] ${SIZES[size] || SIZES.md}${className ? ` ${className}` : ''}`}
      data-phase={phase}
    ...
    >
      <style>{STYLE}</style>
      <span
        className="hb-pulse pointer-events-none absolute inset-0 z-0 opacity-0 rounded-(--hb-radius) group-data-[glow=true]:group-data-[phase=done]:animate-[hb-pulse_600ms_var(--hb-ease-out)_forwards]"
        aria-hidden="true"
      />
      ...
      <span
        ...
        aria-hidden="true"
      >
        <span className="hb-fill absolute inset-0 grid place-items-center [background:var(--hb-fill)] text-(--hb-fill-text)">
          <span className="hb-label grid place-items-center">{labels}</span>
        </span>
        <span className="hb-crest absolute inset-0 grid place-items-center [background:var(--hb-fill)] text-(--hb-fill-text)">
          <span className="hb-label grid place-items-center">{labels}</span>
        </span>
      </span>
