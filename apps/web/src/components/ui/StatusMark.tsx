...
const StatusMark: React.FC<StatusMarkProps> = ({
  ...
  return (
    ...
    >
      <style>{STYLE}</style>
      <svg
        className="shrink-0 overflow-visible text-(--sm-color) [transition:color_200ms_ease] group-group-data-[status=done]:text-(--sm-done)-data-[status=failed]:[color:var(--sm-error)]"
        viewBox="0 0 24 24"
        width={size}
        height={size}
      ...
      {hasLabel ? (
        <span className="relative leading-tight opacity-[0.65] text-(--sm-color) [font-size:var(--sm-font)] [transition:opacity_200ms_ease] group-data-[status=running]:opacity-100 group-data-[status=done]:opacity-60 group-data-[status=failed]:opacity-100 group-data-[status=cancelled]:opacity-[0.55]">
          {label}
          <span
            className="pointer-events-none absolute inset-x-0 top-1/2 origin-left scale-x-0 bg-current [translate:0_-50%] h-[max(1px,calc(var(--sm-font)/14))] [transition:transform_160ms_cubic-bezier(0.23,1,0.32,1)] group-data-[status=done]:group-data-strike:scale-x-100 group-data-[status=done]:group-data-strike:[transition:transform_280ms_cubic-bezier(0.23,1,0.32,1)_var(--sm-strike-delay)] motion-reduce:scale-x-100! motion-reduce:opacity-0 motion-reduce:[transition:opacity_200ms_ease] motion-reduce:group-data-[status=done]:group-data-strike:opacity-100 motion-reduce:group-data-[status=done]:group-data-strike:[transition:opacity_200ms_ease]"
            aria-hidden="true"
          />
        </span>
      ) : null}
