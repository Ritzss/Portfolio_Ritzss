...
const BranchedMenu: React.FC<BranchedMenuProps> = ({
  ...
  return (
    <nav
      ref={navRef}
      className={`relative flex w-fit max-w-[min(var(--bm-w),100%)] flex-col pl-3.5 leading-[1.2] text-(--bm-ink) font-[inherit] [font-size:var(--bm-font)] before:absolute before:top-2 before:bottom-0 before:left-0 before:w-0.5 before:rounded-[1px] before:[background:linear-gradient(to_bottom,var(--bm-line)_0%,var(--bm-line)_55%,transparent_100%)] before:content-['']${className ? ` ${className}` : ''}`}
      style={
        {
          '--bm-w': `${width}px`,
          '--bm-ink': color,
      ...
      {items.map((item, i) => {
        ...
        return (
          ...
          >
            ...
            {kids ? (
              <div className="grid grid-rows-[0fr] [transition:grid-template-rows_var(--bm-fold)_cubic-bezier(0.23,1,0.32,1)] group-data-open/section:grid-rows-[1fr] motion-reduce:transition-none">
                <div className="min-h-0 overflow-hidden">
                  <div className="relative box-border py-1.5" style={{ height: bodyH }}>
                    ...
                    >
                      <path
                        className="fill-none stroke-(--bm-line) [stroke-width:var(--bm-line-w)] [stroke-linecap:round] [stroke-linejoin:round]"
                      ...
                      {kids.map((kid, k) => (
                        <path
                          key={kid.value}
                          className="fill-none stroke-(--bm-accent) [stroke-width:var(--bm-line-w)] [stroke-linecap:round] [stroke-linejoin:round] [transition:stroke-dashoffset_var(--bm-draw)_cubic-bezier(0.23,1,0.32,1)] motion-reduce:transition-none"
                          d={reach(k)}
                          style={{
