...
const TearTicket: React.FC<TearTicketProps> = ({
  ...
  return (
    <div
      ref={rootRef}
      className={`group relative select-none w-[min(var(--tt-w),100%)] text-(--tt-ink) [-webkit-tap-highlight-color:transparent] [-webkit-touch-callout:none] data-[disabled]:opacity-60${className ? ` ${className}` : ''}`}
    ...
    >
      <div
        ref={stageRef}
        className="absolute top-0 left-0 origin-top-left w-(--tt-w) h-(--tt-h) transform-[scale(var(--tt-fit))] [transition:transform_650ms_cubic-bezier(0.22,1,0.36,1)] group-data-[shift=x]:transform-[translateX(calc(var(--tt-stub)*var(--tt-fit)/2))_scale(var(--tt-fit))] group-data-[shift=y]:transform-[translateY(calc(var(--tt-stub)*var(--tt-fit)/2))_scale(var(--tt-fit))] group-data-instant:duration-[0ms] motion-reduce:[transition:none]"
      >
        <motion.div className="absolute inset-0" style={{ transform: plane }}>
          <div ref={bodyRef} className="pointer-events-none absolute inset-0">
            {border ? (
              <svg
                className="pointer-events-none absolute inset-0 h-full w-full overflow-visible [&>path]:fill-none [&>path]:stroke-(--tt-edge) [&>path]:[stroke-width:var(--tt-edge-w)]"
            ...
            >
              {image ? (
                <div className="absolute overflow-hidden top-(--tt-inset) left-(--tt-inset) w-[calc(var(--tt-body-w)-var(--tt-inset)*2)] h-[calc(var(--tt-body-h)*var(--tt-span)-var(--tt-inset)*2)] rounded-(--tt-art-radius)">
                  <motion.img
                    className="absolute max-w-none object-cover -top-(--tt-parallax) left-[calc(valeft-[calc(var(--tt-parallax)*-1)]100%+var(--tt-parallax)*2)] h-[calc(100%+var(--tt-parallax)*2)] [-webkit-user-drag:none] [transition:filter_700ms_ease] group-data-used:filter-[grayscale(1)_brightness(0.75)] group-data-instant:duration-[0ms]"
                    src={image}
                    alt={imageAlt}
                    draggable={false}
              ...
              <motion.div
                className="absolute inset-y-0 left-0 w-[calc(var(--tt-w)-var(--tt-stub))] [transition:opacity_500ms_ease] group-data-used:opacity-55 group-data-instant:duration-[0ms] group-data-[orientation=vertical]:bottom-auto group-data-[orientation=vertical]:w-full group-data-[orientation=vertical]:h-[calc(var(--tt-h)-var(--tt-stub))]"
          ...
          <svg
            className="pointer-events-none absolute inset-0 h-full w-full overflow-visible [&_path]:fill-none [&_path]:opacity-0 [&_path]:stroke-(--tt-bg) [&_path]:[stroke-linecap:round]"
            aria-hidden="true"
          >
            {geo.bridges.map((b, i) => (
              <g key={i}>
          ...
          >
            ...
            >
              <div className="absolute inset-y-0 right-0 w-(--tt-stub) group-data-[orientation=vertical]:top-auto group-data-[orientation=vertical]:left-0 group-data-[orientation=vertical]:w-auto group-data-[orientation=vertical]:h-(--tt-stub)">
                {stub}
              </div>
            </div>
          </div>
