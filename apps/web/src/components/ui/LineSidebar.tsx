...
const LineSidebar = ({
  ...
  return (
    <nav
      className={`relative flex justify-start${showMarker ? ' pl-[calc(var(--marker-length)+var(--marker-gap))]' : ''}${className ? ` ${className}` : ''}`}
      style={
        {
      ...
      >
        {items.map((label, index) => (
          ...
          >
            ...
            <span className="relative inline-flex items-baseline leading-[1.2] text-[color-mix(in_srgb,var(--accent-color)_calc(var(--effect,0)*100%),var(--text-color))] [font-size:var(--font-size)] transform-[translateX(calc(var(--effect,0)*var(--max-shift)))]">
              {showIndex && (
