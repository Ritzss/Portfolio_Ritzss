...
const FuseButton: React.FC<FuseButtonProps> = ({
  ...
  const line = (
    <i
      ref={lineRef}
      className="pointer-events-none absolute inset-x-0 bottom-0 h-(--fb-fuse-h) origin-left [background:var(--fb-fuse)] [box-shadow:0_0_6px_color-mix(in_srgb,var(--fb-fuse)_55%,transparent)] group-data-[fuse=top]:top-0 group-data-[fuse=top]:bottom-auto forced-colors:[background:Highlight]"
      aria-hidden="true"
    />
  );
  ...
  return (
    <span
      ref={rootRef}
      tabIndex={-1}
      className={`group relative inline-grid h-(--fb-h) grid-cols-[minmax(0,1fr)] isolate touch-manipulation select-none overflow-hidden rounded-(--fb-radius) font-medium leading-none tracking-[0.01em] outline-none font-[inherit]size:var(--fb-fs)] [background:var(--fb-bg)] text-(--fb-ink) [-webkit-touch-callout:none] [-webkit-tap-highlight-color:transparent] [transition:transform_160ms_var(--fb-ease-out)] data-[data-pressed:transform-[scale(var(--fb-press))]n-reduce:data-[pressed]:[transform:none] has-focus-visible:outline-2 has-focus-visible:outline-solid has-focus-visible:outline-offset-[3px] has-focus-visible:outline-[color-mix(in_srgb,var(--fb-ink)_60%,transparent)] data-[phase=idle]:has-[.fb-idle:disabled]:opacity-[0.55] contrast-more:[box-shadow:inset_0_0_0_1px_color-mix(in_srgb,var(--fb-ink)_40%,transparent)]${className ? ` ${className}` : ''}`}
      data-phase={phase}
      data-fuse={fuse}
    ...
    >
      ...
      >
        <span
          className="inline-flex h-(--fb-icon) w-(--fb-icon) [&>svg]:h-full [&>svg]:w-full"
          aria-hidden="true"
        >
          {actionIcon}
        </span>
      ...
      >
        <span
          className="inline-flex h-(--fb-icon) w-(--fb-icon) [&>svg]:h-full [&>svg]:w-full transform-[rotate(-70deg)] [transition:transform_var(--fb-fade)_var(--fb-ease-out)] group-data-[phase=armed]:transform-[rotate(0deg)] motion-reduce:transition-none motion-reduce:transform-none"
          aria-hidden="true"
        >
      ...
      {fuse === 'outline' ? (
        <svg
          className="pointer-events-none absolute inset-0 h-full w-full overflow-visible opacity-0 filter-[drop-shadow(0_0_3px_color-mix(in_srgb,var(--fb-fuse)_60%,transparent))] [transition:opacity_var(--fb-fade)_ease] group-data-[phase=armed]:opacity-100 group-data-instant:duration-[0s]"
          aria-hidden="true"
        >
          <rect
            ref={rimRef}
            pathLength="1"
            className="fill-none [x:calc(var(--fb-fuse-h)/2)] [y:calc(var(--fb-fuse-h)/2)] w-[calc(100%-var(--fb-fuse-h))] h-[calc(100%-var(--fb-fuse-h))] [rx:max(0px,calc(var(--fb-radius)-var(--fb-fuse-h)/2))] stroke-(--fb-fuse) [stroke-width:var(--fb-fuse-h)] [stroke-linecap:round] [stroke-dasharray:1] forced-colors:stroke-[Highlight]"
          />
        </svg>
      ) : null}
