...
const PeekRating: React.FC<PeekRatingProps> = ({
  ...
  const indexAt = (x: number, y: number): number | null => {
    ...
    if (pressing && (y < rect.top - size || y > rect.bottom + size))
      return null;
    const index = clamp(
      Math.floor(((x - rect.left) / rect.width) * count),
      0,
      count - 1,
    );
  ...
  return (
    ...
    >
      <div
        ref={rowRef}
        className="relative inline-flex touch-pan-y select-none items-end pt-(--pr-room) [-webkit-tap-highlight-color:transparent] [-webkit-touch-callout:none]"
      ...
      >
        {interactive && showTip ? (
          <span
            ref={tipEl}
            className="pointer-events-none absolute left-0 top-0 inline-flex h-[calc(var(--pr-size)*0.6)] items-center whitespace-nowrap rounded-full px-[calc(var(--pr-size)*0.3)] text-[calc(var(--pr-size)*0.38)] font-semibold leading-none tracking-[0.01em] opacity-0 shadow-[0_2px_10px_rgba(0,0,0,0.18)] [background:var(--pr-tip)] text-(--pr-tip-text) [transition:transform_var(--pr-rise)_var(--pr-ease-out),opacity_180ms_ease] data-[show=true]:opacity-100 group-data-[instant=true]:duration-[0ms] motion-reduce:[transition:opacity_180ms_ease]"
            aria-hidden="true"
          />
        ) : null}
        {Array.from({ length: count }, (_, i) => {
          ...
          return (
            <Star
              ...
              className="m-0 grid h-[calc(var(--pr-size)+8px)] w-[calc(var(--pr-size)+var(--pr-gap))] touch-manipulation place-items-center border-0 bg-transparent p-0 font-[inherit] text-inherit outline-none [@media(hover:hover)_and_(pointer:fine)]:cursor-none pointer-coarse:min-h-11 [@pointer-coarse:min-w-11ocus-visible:rounded-md focus-visible:outline-offset-2 focus-visible:[outline:2px_solid_color-mix(in_srgb,var(--pr-active)_70%,transparent)]"
              role={readOnly ? undefined : "radio"}
              aria-checked={readOnly ? undefined : checked}
              aria-label={readOnly ? undefined : label}
              aria-hidden={readOnly || undefined}
              tabIndex={
                !interactive ? -1 : (value === 0 ? i === 0 : checked) ? 0 : -1
              }
            >
              <span
                ref={(el) => {
                ...
                className="block origin-bottom [transition:transform_var(--pr-rise)_var(--pr-ease-out)] group-data-[instant=true]:duration-[0ms] motion-reduce:transition-none"
              >
                <span
                  ref={(el) => {
                    glyphEls.current[i] = el;
                  }}
                  className="block h-(--pr-size) w-(--pr-size) text-(--pr-idle) [transition:color_160ms_ease] data-[lit=true]:text-(--pr-active) data-[instant=true]:duration-[0ms] [&>svg]:block [&>svg]:h-full [&>svg]:w-full"
                >
                  {icon ?? (
                    <HugeiconsIcon
                      icon={shapeIcon}
